...
module Data.HES.Messages.TransactionWriteCompleted (TransactionWriteCompleted(..)) where
import Prelude ((+), (/))
import qualified Prelude as Prelude'
import qualified Data.Typeable as Prelude'
import qualified Data.Data as Prelude'
import qualified Text.ProtocolBuffers.Header as P'
import qualified Data.HES.Messages.OperationResult as HES.Messages (OperationResult)
 
data TransactionWriteCompleted = TransactionWriteCompleted{transaction_id :: !P'.Int64, result :: !HES.Messages.OperationResult,
                                                           message :: !(P'.Maybe P'.Utf8)}
                               deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable TransactionWriteCompleted where
  mergeAppend (TransactionWriteCompleted x'1 x'2 x'3) (TransactionWriteCompleted y'1 y'2 y'3)
   = TransactionWriteCompleted (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)
 
instance P'.Default TransactionWriteCompleted where
  defaultValue = TransactionWriteCompleted P'.defaultValue P'.defaultValue P'.defaultValue
 
instance P'.Wire TransactionWriteCompleted where
  wireSize ft' self'@(TransactionWriteCompleted x'1 x'2 x'3)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(TransactionWriteCompleted x'1 x'2 x'3)
   = case ft' of
       10 -> put'Fields
       11 -> do
