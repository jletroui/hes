module System.HES.Internal.Client(
  newClient,
  createStream) where

import System.HES.Internal.ESPacket
import qualified Data.HES.MessageCodes as Msg
import Data.HES.Messages.CreateStream
...
newClient port host = do
  ...
  runTCPClient settings $ \server -> do
    ...
    _ <- forkIO $ (fromClientSource client) $= fromESPacketConduit $$ (appSink server)
    return ()
  return client
